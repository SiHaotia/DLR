...
class spatial_prior_encoder_gelu(nn.Module):

    def __init__(self, in_chans=6, embed_dim=64, block_num=4, stage=1, group=4, patch_expansion=0.5, channel_expansion=4):
        super(spatial_prior_encoder_gelu, self).__init__()

        # assert in_chans == int(12//stage), "in chanel size is wrong"
        self.group = group

        self.conv1 = nn.Sequential(
                nn.Conv2d(in_chans, in_chans*16, 7, 4, 3),
                nn.GELU(),
    ...
    def forward(self, inp_img, gt=None):        
        ...
        x = self.mlp(x)
        x = self.end(x)
        type = self.type_mlp(x).squeeze()
        return x, type

@ARCH_REGISTRY.register()
class spatial_prior_encoder_lrelu(nn.Module):

    def __init__(self, in_chans=6, embed_dim=64, block_num=4, stage=1, group=4, patch_expansion=0.5, channel_expansion=4):
        super(spatial_prior_encoder_lrelu, self).__init__()
