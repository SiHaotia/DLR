...
class DDPM(nn.Module):
    ...
    def forward(self, img_ir, img_vi, prior_condition=None, x=None):
        # b, c, h, w, device, img_size, = *x.shape, x.device, self.image_size
        # assert h == img_size and w == img_size, f'height and width of image must be {img_size}'
        img = torch.cat([img_ir,img_vi], dim=1)
        device = self.betas.device
        b=img.shape[0]
        if self.training:
            pred_prior_list=[]
            ...
            prior = x_noisy
            for i in reversed(range(0, self.num_timesteps)): ## 直接输出了每一步的噪声 而不是随机采样步数
                prior, predicted_noise = self.p_sample(prior, torch.full((b,), i,  device=device, dtype=torch.long), prior_condition,
                                clip_denoised=self.clip_denoised)
            ...
            for i in reversed(range(0, self.num_timesteps)):
                prior, _ = self.p_sample(prior, torch.full((b,), i,  device=device, dtype=torch.long), prior_condition,
                                clip_denoised=self.clip_denoised)
            return prior
