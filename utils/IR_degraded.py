...
if __name__ == '__main__':
    preprocessor = SRMDPreprocessing(
        ...
        lambda_2=4.0,  # 非各向同性高斯模糊参数
        theta=0,  # 非各向同性高斯模糊参数
        lambda_min=0.2,  # 非各向同性高斯模糊参数
        lambda_max=4.0,  # 非各向同性高斯模糊参数
        noise=0  # 噪声水平
    )
    # 输入文件夹路径和输出文件夹路径
    input_folder = "datasets/MSRS/train/ir_noise1"
    output_folder = "datasets/MSRS/train/ir_noise"
    low_output_folder = "datasets/MSRS/train/ir_noise_low"
    ...
    for filename in filebar:
        if filename.endswith('.jpg') or filename.endswith('.png'):
            ...
            # 处理图像
            lr_blured, _ = preprocessor(image_tensor, random=True)
